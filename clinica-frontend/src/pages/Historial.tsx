...
export default function Historial() {
  ...
  return (
    <div className="w-full max-w-full min-w-0 px-4 sm:px-6 lg:px-8 py-8 animate-fade-in-up overflow-x-hidden">
      ...
      {/* ENCABEZADO */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-5 min-w-0">
        <div className="min-w-0">
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight font-serif flex items-center gap-3">
            <span className="p-2 bg-slate-100 rounded-xl text-slate-600 shrink-0"><Icons.History /></span>
        ...
        <div className="relative w-full lg:w-[360px] xl:w-[420px] shrink-0">
          ...
          <input
            ...
            placeholder="Buscar por paciente, expediente, diagnóstico..."
            value={busqueda}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden w-full max-w-full min-w-0">
        <div className="w-full max-w-full overflow-x-auto">
          <table className="table table-fixed w-full min-w-[940px]">
            <colgroup>
            ...
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-xs font-bold text-slate-500 uppercase tracking-wider text-center pr-4">Detalle</th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && registros.map((reg) => {
                ...
                return (
                  <tr key={reg.ID_Sesion} className="hover:bg-slate-50 transition-colors group align-top">
                    ...
                    <td className="py-4">
                      <div className="flex flex-col min-w-0">
                        <span className="font-bold text-slate-800 text-sm group-hover:text-blue-600 transition-colors truncate" title={nombrePaciente}>
                          {nombrePaciente}
                        </span>
                        <div className="flex items-center gap-2 mt-1 min-w-0">
                          ...
                          <span className="text-[10px] text-slate-400 uppercase tracking-wide truncate" title={nacionalidad}>
                            {nacionalidad}
                          </span>
                        </div>
                      </div>
                    ...
                    <td className="py-4">
                      <div className="flex flex-col gap-1 min-w-0">
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-700 w-fit max-w-full uppercase tracking-wide truncate">
                          {tipoCita}
                        </span>
                        <span className="text-sm text-slate-600 truncate" title={motivoConsulta}>
                          {motivoConsulta}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-4 whitespace-nowrap">
                      <div className="text-xs font-medium text-slate-500 flex items-center gap-2 min-w-0">
                    ...
                    <td className="py-4 text-center pr-4">
                      ...
                      <dialog id={`modal_nota_${reg.ID_Sesion}`} className="modal modal-bottom sm:modal-middle backdrop-blur-sm text-left">
                        <div className="modal-box bg-white p-0 rounded-2xl overflow-hidden max-w-3xl">
                          <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center gap-4">
                            <div className="min-w-0">
                              <h3 className="font-bold text-lg text-slate-800 truncate">Nota Clínica</h3>
                              <p className="text-xs text-slate-500 truncate">{nombrePaciente} · {formatearFecha(reg.FechaReal)}</p>
                            </div>
                            <form method="dialog">
                          ...
                          <div className="p-6 max-h-[65vh] overflow-y-auto space-y-6">
                            <section>
                              <h4 className="font-bold text-slate-700 text-sm mb-2 uppercase tracking-wider">Motivo de consulta</h4>
                              <p className="text-sm text-slate-600 bg-slate-50 border border-slate-100 rounded-xl p-3 whitespace-pre-wrap">
                                {motivoConsulta}
                              </p>
                            </section>

                            <section>
                              <h4 className="font-bold text-slate-700 text-sm mb-2 uppercase tracking-wider">Diagnóstico diferencial</h4>
                              <p className="text-sm text-slate-600 bg-blue-50/50 border border-blue-100 rounded-xl p-3 whitespace-pre-wrap">
                            ...
                            <section>
                              <h4 className="font-bold text-slate-700 text-sm mb-2 uppercase tracking-wider">Historial de evolución</h4>
                              <p className="text-sm text-slate-600 bg-amber-50 border border-amber-100 rounded-xl p-3 whitespace-pre-wrap">
                            ...
                            {reg.Criterios_DeDiagnostico && (
                              <section>
                                <h4 className="font-bold text-slate-700 text-sm mb-2 uppercase tracking-wider">Criterios de diagnóstico</h4>
                                <p className="text-sm text-slate-600 bg-slate-50 border border-slate-100 rounded-xl p-3 whitespace-pre-wrap">
                                  {reg.Criterios_DeDiagnostico}
                                </p>
                              </section>
