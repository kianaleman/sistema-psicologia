  ...
  return (
    <div className="p-8 animate-fade-in-up max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
        ...
        <button 
          className="btn btn-primary shadow-lg text-white gap-2 rounded-xl px-6 hover:scale-105 transition-transform" 
          onClick={openCreate}
        >
          <Icons.Plus />
          Nuevo Paciente
                    ...
                    <tr key={p.ID_Paciente} className="hover:bg-slate-50 transition-colors group">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5">
                                        <span className={`w-2 h-2 rounded-full ${p.Activo ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                                        <span className="text-xs text-slate-500 font-medium">{p.Activo ? 'Activo' : 'Inactivo'}</span>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td className="py-4">
                            <div className="text-sm font-medium text-slate-600 font-mono">
                                {/* CORRECCIÓN: Ajustado a p.Paciente_Menor */}
                                {p.PacienteAdulto ? p.PacienteAdulto.No_Cedula : p.Paciente_Menor?.PartidaDeNacimiento || 'Sin identificación'}
                            </div>
                        ...
                        <td className="pr-6 py-4 text-right">
                            <div className="flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                <Link to={`/pacientes/${p.ID_Paciente}`} className="btn btn-sm btn-ghost text-blue-600 hover:bg-blue-50 tooltip tooltip-left" data-tip="Ver Expediente">
                                    <Icons.Folder />
                                </Link>
                                <button className="btn btn-sm btn-ghost text-slate-500 hover:bg-slate-100 tooltip tooltip-left" data-tip="Editar Datos" onClick={() => openEdit(p)}>
                                    <Icons.Edit />
                                </button>
                            </div>
                        </td>
